...
def find_next_verts(last_image, list_verts, array_availability_verts):
    global MIN_SCORE
    next_id = find_next_image(last_image, list_verts, array_availability_verts)
    next_vert = list_verts[next_id]
    baseline_score = compute_score_transition(last_image[1], next_vert[1])
    array_availability_verts[next_id] = 0
    list_available = np.where(array_availability_verts == 1)[0]
    max_score = 0
    best_id = "bla"
    for idx in list_available:
        tag_length = len(last_image[1])
        tag_length_2 = len(list_verts[idx][1])+len(next_vert[1])
        if tag_length < 3:
            break
        score = compute_score_transition(last_image[1],
                                         combine_verts(next_vert, list_verts[idx])[1])
        if score >= min([baseline_score+MIN_SCORE, tag_length//2, tag_length_2//2]):
            return next_id, idx
        if score >=max_score:
            max_score=score
            best_id=idx
...
def combine_verts(vert_1, vert_2):
    return (str(vert_1[0])+" "+str(vert_2[0]), list(set(vert_1[1]+vert_2[1])))
...
def main():
    ...
    list_verts = list(zip(vertical_ids, verticals))
    cat = [(len(tags), (id, tags) ) for id, tags in list_verts]
    z = sorted(cat,key=lambda x:x[0])[::-1]
    list_verts = [item[1] for item in z]
    ...
    if "shiny" in file_name:
        array_availability_verts = np.ones(len(list_verts))
        first_slide_img = combine_verts(list_verts[0], list_verts[1])
        array_availability_verts[0]=0
        array_availability_verts[1]=0
        last_image = first_slide_img
        for i in tqdm(range(len(list_verts)//2-1)):
            id1, id2 = find_next_verts(last_image, list_verts, array_availability_verts)
            array_availability_verts[id1] = 0
            array_availability_verts[id2] = 0
            new_slide = combine_verts(list_verts[id1], list_verts[id2])
            result_list += [str(new_slide[0])]
            if i%1000==0:
    ...
    lh = lh + get_horiz_from_vert(list_verts)
    #sort by num tags
